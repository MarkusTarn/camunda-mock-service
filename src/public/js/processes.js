...
async function enableFileUpload(id) {
...
async function getProcessesAsync() {
    const processes = await fetch('http://localhost:8080/engine-rest/process-definition')
...
async function startProcessAsync(id) {
    ...
    return await fetch('http://localhost:8080/engine-rest/process-definition/' + id + '/start', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
    })
        .then(response => response.json())
        .then(document.getElementById(id + '-count').innerHTML = parseInt(document.getElementById(id + '-count').innerHTML) + 1)
        .then(M.toast({ html: 'Process started' }))
        .catch(error => alert(error));
}
...
async function addInstanceCountAsync(process) {
    return await fetch('http://localhost:8080/engine-rest/process-instance/count?processDefinitionId=' + process.id)
        .then(response => response.json())
        .then(data => {
            ...
            <div class="row">
                <div class="col s6">
                    <div class="file-field input-field">
                        <div class="btn">
                            <span>Model</span><input id="${process.id + '-file'}" type="file" onchange="enableFileUpload('${process.id}');">
                        </div>
                ...
                <div class="col s6">
                    ...
                    <button class="btn waves-effect waves-light right disabled" id="${process.id + '-update'}" onclick="updateProcessAsync('${process.id}');">Update<i class="material-icons right">build</i></button>
                </div>
            </div>
        </div>
    </li>`
